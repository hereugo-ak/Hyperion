...
class FinancialAnalyst(BaseAgent):
    ...
    async def _pull_macro_inputs(self, geography: str = "") -> dict[str, Any]:
        ...
        if not is_us:
            # FRED is US-only. Route international requests to World Bank
            # instead of merely labelling US figures as a geography mismatch.
            try:
                ...
                macro.update({
                    ...
                    "risk_free_rate_unavailable": (
                        "World Bank does not provide a sovereign risk-free yield; "
                        "a country-specific market source is required"
                    ),
                })
                self._sources.append(Source(
                    id=f"src_{len(self._sources):03d}",
